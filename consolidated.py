...
class LicenseOutput(BaseModel):
    ...
    first_name: str = Field(description="Extract First name in the name. The first name corresponds to entry number 2 on the license,Return string with value NULL if you cannot identify ")
    ...
    expiry_date : str = Field(description="What is the Expiry Date of the License,Return string with value  NULL if you cannot identify")
    ...
    license_number: str = Field(description="Find out the license number,Return string with value NULL if you cannot identify")
...
def passport_verify(image_path, first_name, last_name):
    ...
    message = HumanMessage(
        content=[
            {"type": "text", "text": "Verify whether the following document is a passport. Give me Verification as a boolean, First Name, Last Name and date as YYYY-MM-DD in the image. Make the output passable to Json Output Parser"},
            {
                "type": "image_url",
                "image_url": {"url": f"data:image/jpeg;base64,{image_data}"},
            },
    ...
    if any(field.upper() == "NULL" for field in [response.first_name, response.last_name, response.expiry_date, response.nationality, response.passport_number]):
        return 0
...
def license_verify(image_path, first_name, last_name):
    ...
    message = HumanMessage(
        content=[
            {"type": "text", "text": "Verify whether the following document is a driving license. Give me Verification as a boolean, First Name, Last Name and date as YYYY-MM-DD in the image. Make the output passable to Json Output Parser"},
            {
                "type": "image_url",
                "image_url": {"url": f"data:image/jpeg;base64,{image_data}"},
            },
    ...
    if any(field.upper() == "NULL" for field in [response.first_name, response.last_name, response.expiry_date, response.country, response.license_number]):
        return 0
